...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MATLAB Script for Pre-Processing Merck Test Dataset
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1. Get the tumor middle slice
% 2. Crop tumor bbox (<70pixels: 70x70 ; >70pixels: resample->70x70)

%% load raw 3d data
data_dir = '../../Data_Segmentation/merck_test_data/Raw_DATA/TEST/';
files = dir(data_dir);
save_dir = '../../Data_Segmentation/merck_test_data/';

fileID_test = fopen(strcat(save_dir,'dir/','test_list_merck.txt'),'wt');
...
for i = 3:4:length(files)
    ...
    if contains(filename, '_CT.hdr')
        file_dir = strcat(files(i).folder, '/',...
            strrep(filename, '_CT.hdr','_CT.img'));
    ...
    if contains(filename, '_CT.mask_edited.hdr')
        file_dir = strcat(files(i+2).folder, '/',...
            strrep(filename, '_CT.mask_edited.hdr','_CT.mask_edited.img'));
        V_label = analyze75read(file_dir);
    end
    V_label = uint16(uint16(V_label)>=1);
    ...
    for zz = min_z:max_z     
        ...
        img_file_name = strcat(strrep(filename, '_CT.mask_edited.hdr', '_img_test_'), string(zz), '.png');
        mask_file_name = strcat(strrep(filename, '_CT.mask_edited.hdr', '_mask_test_'), string(zz), '.png');
        edge_file_name = strcat(strrep(filename, '_CT.mask_edited.hdr', '_edge_test_'), string(zz), '.png');
