...
def main():
    ...
    criterion_rg = nn.CrossEntropyLoss().cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion_da, criterion_rg, optimizer, epoch, data_logger=data_logger, class_names=class_names)
        
        # evaluate on validation set
        if epoch % args.ef == 0 or epoch == args.epochs:
            m = validate(val_loader, model, criterion_da, criterion_rg, epoch, data_logger=data_logger, class_names=class_names)
...
def train(train_loader, model, criterion_da, criterion_rg, optimizer, epoch, data_logger=None, class_names=None):
    ...
    for i, (case_ind, input, mask, edge, class_vec) in enumerate(train_loader):
        ...
        loss_ba = criterion_da(output_da, edge_var)
        ...
        mask_np = mask_var.data.cpu().numpy()[:, np.newaxis, :, :]
        ...
        if epoch % args.df == 0:
            ...
            output_ba_disp = make_tf_disp_slice(output_ba_np[0, 1, :, :], edge_var.data.cpu().numpy()[0, :, :])
            output_rg_disp = make_tf_disp_slice(output_rg_np[0, 1, :, :], mask_var.data.cpu().numpy()[0, :, :])
            output_fin_disp = make_tf_disp_slice(output_fin_np[0, 1, :, :], mask_var.data.cpu().numpy()[0, :, :])
