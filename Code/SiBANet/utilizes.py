...
"""Save the output volume to npy for analysis (CT volume, SiBA segmentation, GT segmentation)"""
def save_seg_npy(input, output, target, ind_all, savefolder):
    if output.shape != target.shape:
        raise ValueError("Shape mismatch: Image & Prediction & Ground-Truth must have the same shape!")
    ...
    for i in range(ind_all.min(), ind_all.max() + 1):
        ...
        np.save(file=savefolder + str(i) + '_vol.npy', arr=vol_input)
        np.save(file=savefolder + str(i) + '_gt.npy', arr=vol_gt)
        np.save(file=savefolder + str(i) + '_pred.npy', arr=vol_output)
...
def predict_unlabel(unlabel_loader, model, percent_keep, network='SiBA'):
