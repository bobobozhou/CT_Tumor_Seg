...
def main():
    ...
    m = validate(val_loader, model, criterion, epoch=1, data_logger=data_logger, class_names=class_names)
...
def validate(val_loader, model, criterion, epoch, data_logger=None, class_names=None):
    ...
    for i, (case_ind, input, mask, edge, class_vec) in enumerate(val_loader):
        ...
        np.save('input_var', input_var.data.cpu().numpy()[5,:,:,:])
        np.save('output_fin', output_fin.data.cpu().numpy()[5,:,:,:])
        ipdb.set_trace()
